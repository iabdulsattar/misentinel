...
export class DobFeedComponent implements OnInit {
  ...
  loadFilterOptions(): void {
    ...
    this.edobService.listEntryTypes(orgId).subscribe((types: EntryType[]) => this.entryTypes = types);
    this.edobService.listOrgUsers(orgId).subscribe((users: OrgUser[]) => this.orgUsers = users);
  ...
  loadEntries(): void {
    ...
    this.edobService.listEntries(orgId, filters).subscribe({
      next: (res: ListEntriesResponse) => {
        this.entries = this.mapEntries(res.entries);
  ...
  private mapEntries(raw: Entry[]): any[] {
    ...
    return raw.map(entry => {
      const typeName = typeMap.get(entry.entryTypeCode) || entry.entryTypeCode;
