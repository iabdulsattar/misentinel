...
export class AIGenerationService {
  ...
  private buildSystemPrompt(entryTypeCode: EntryTypeCode, categories: string[] = [], priorities: string[] = []): string {
    ...
    if (entryTypeCode === 'BASIC') {
      ...
      schema = `Required JSON schema: "title" (concise handover title), "category" (choose from available categories), "priority" (choose from available priorities), "handoverType" (choose from available handover types), "description" (operational summary), "data" object with "outstandingIssues" (array of unresolved issues), "outstandingActions" (array of action task objects with "task" and "done" boolean), "importantInformation" (key notes for next shift).`;
